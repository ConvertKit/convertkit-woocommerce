 ...
 * settings when the Subscribe Event = Order Processing, and an order is placed through WooCommerce
 * 
 * @since 	1.9.6
 */
class PurchaseDataOnOrderProcessingEventCest
{
	/**
	...
	public function testSendPurchaseDataWithSimpleProduct(AcceptanceTester $I)
	{
		...
		$result = $I->wooCommerceCreateProductAndCheckoutWithConfig(
			...
			'Order Processing', // Perform on WooCommerce "Order Processing" event (not used)
			true // Send purchase data to ConvertKit
		);

		// Confirm that the purchase was not yet added to ConvertKit.
		$I->apiCheckPurchaseDoesNotExist($I, $result['order_id'], $result['email_address']);

		// Change the Order Status
		$I->wooCommerceChangeOrderStatus($I, $result['order_id'], 'wc-processing');
